...
interface Notification {
  id: string; titleTh: string; titleEn: string;
  descTh: string; descEn: string; time: string;
  unread: boolean; type: "survey" | "system" | "security";
}

const MOCK_NOTIFICATIONS: Notification[] = [
  { id:"n1", titleTh:"แบบสำรวจใหม่",   titleEn:"New survey available",  descTh:"Pulse Survey Q2 2025 เปิดให้ตอบแล้ว",        descEn:"Pulse Survey Q2 2025 is now open",           time:"2m", unread:true,  type:"survey"   },
  { id:"n2", titleTh:"ใกล้ปิดรับ",      titleEn:"Survey closing soon",   descTh:"Annual Engagement จะปิดใน 3 วัน",           descEn:"Annual Engagement closes in 3 days",         time:"1h", unread:true,  type:"system"   },
  { id:"n3", titleTh:"รายงานพร้อม",    titleEn:"Report ready",           descTh:"Executive Summary Q2 พร้อมดาวน์โหลด",       descEn:"Executive Summary Q2 ready to download",    time:"1d", unread:false, type:"system"   },
  { id:"n4", titleTh:"ตอบครบเป้า",     titleEn:"Target reached",         descTh:"แผนก IT ตอบกลับครบ 80%",                   descEn:"IT department reached 80% response rate",   time:"2d", unread:false, type:"security" },
];
                  ...
                  <div>
                    {notifications.map((n) => (
                      <button key={n.id} type="button" className={cn(
                        ...
                        n.unread && "bg-indigo-50/40 dark:bg-indigo-950/30"
                      )}>
                        {n.unread && <div className="absolute left-0 top-0 bottom-0 w-0.5 bg-indigo-500 rounded-r" />}
                        <div className="flex items-start gap-3">
                          <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center shrink-0",
                            n.type === "survey" ? "bg-indigo-100 dark:bg-indigo-900 text-indigo-600 dark:text-indigo-400" :
                            n.type === "security" ? "bg-rose-100 dark:bg-rose-900 text-rose-600 dark:text-rose-400" :
