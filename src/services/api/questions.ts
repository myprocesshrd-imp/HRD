    ...
    if (!error && sections && sections.length > 0) {
      ...
      for (const sec of sections) {
        const { data: questions, error: qErr } = await supabaseAdmin
          .from("questions")
          .select("*, question_choices(*), matrix_rows(*), matrix_columns(*), creator:created_by(name_en, name_th), editor:updated_by(name_en, name_th)")
        ...
        result.push({
          ...
          descEn: sec.desc_en ?? "",
          descTh: sec.desc_th ?? "",
          businessUnitId: sec.business_unit_id ?? undefined,
          questions: (questions ?? []).map((q: any) => ({
            ...
            // Audit fields
            createdBy: q.creator ? (q.creator.name_th || q.creator.name_en) : undefined,
            createdAt: q.created_at ?? undefined,
            updatedBy: q.editor ? (q.editor.name_th || q.editor.name_en) : undefined,
